...
code = code.replace(/opsChartData\.map\(\(row, i\)/g, 'opsChartData.map((row: any, i)');
code = code.replace(/opsChartData\.map\(\(entry, index\)/g, 'opsChartData.map((entry: any, index: number)');
